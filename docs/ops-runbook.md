...
- **환불 반영 지연 한계**: 위젯은 다음 성공적인 `syncEntitlements` 이후에만 환불/회수 상태를 안다. 1초 틱/`buildRemoteViews`에서 Play 동기화나 네트워크 호출은 금지한다. 앱 시작·구매 복원·12시간 주기 `WorkManager` 백그라운드 동기화가 성공하면 권한 없는 다운로드 유료 스킨을 로컬에서 삭제하고, 현재 적용 중이면 캐릭터/타이머 각각 내장 무료 스킨으로 fallback 후 `WidgetUpdater.updateAllWidgets`를 호출한다. OS 배터리/네트워크 제약으로 백그라운드 실행은 즉시성을 보장하지 않는다.
