 ...
 *
 * 폴더 규칙 (skinId 기준): 테마별 표시 에셋을 character/preview/{id}/ 한 폴더에 모은다.
 *   character/preview/{id}/thumb.png     ← 테마 썸네일(고정 파일명, 상점 목록/타이머 탭 공용)
 *   character/preview/{id}/prev01.* …    ← 미리보기. prev01,02,03… 가변 개수. 확장자는 칸마다 자유(.png/.gif 혼용 가능, [PREVIEW_EXTS] 순으로 탐침).
 *
...
object SkinRepoUrls {
    ...
    fun themeThumb(skinId: String, base: String = ASSET_BASE) = "$base/character/preview/$skinId/thumb.png"
    ...
    fun previewCandidates(skinId: String, index: Int, base: String = ASSET_BASE): List<String> =
        PREVIEW_EXTS.map { ext -> "$base/character/preview/$skinId/prev%02d.$ext".format(index) }
}
