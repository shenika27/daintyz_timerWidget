# daintyz_timerWidget — 마이 미니 타이머(My mini Timer)

안드로이드 홈 화면 위젯 형태의 "마이 미니 타이머(My mini Timer)" 앱.
...
위젯 동작·스킨 시스템·테마 상점/미리보기·앱 셸, Google Play Billing 기반 유료 테마/평생이용권,
결제 검증 Worker 보호 다운로드가 구현된 단계입니다.
현재 앱 버전은 `0.1.8`(`versionCode` 9)입니다.
...
- **스킨 테마**: 박스·구분선은 스킨의 `timer.background` PNG에 포함, 버튼 5종 + 상황별 캐릭터로 구성.
...
- **앱 화면(Compose)**: 하단 탭 셸(보유 / 상점 / 설정). 카드 탭 → 상세/미리보기 화면.
  ...
  - **설정**: 한 번에 조절(증감) 단위, 위젯 배치, 완료음/진동, 완료 알림 반복,
    시스템 알림 설정 딥링크, 앱 글꼴(기본/시스템), 앱 언어(한국어/English),
    위젯 새로고침, 기프트코드 해금, 앱 버전.

## 기술 스택 (확정)

- 언어: Kotlin
- 앱 지원 언어: 한국어 / English (`AppCompatDelegate` per-app language)
...
- 스킨 배포: GitHub raw catalog + jsDelivr 공개 에셋. 유료 zip은 비공개 R2에 두고 결제 검증 Worker를 통해 보호 다운로드.
...
- **완료 피드백**: 완료 알림 채널은 무음·무진동으로 두고, 완료음·진동은 앱 설정에 따라 직접 재생한다.
  반복 설정이 켜져 있으면 설정 시간 동안 소리·진동을 반복하고, 사용자가 완료를 확인하거나 알림을 지우면 반복을 멈춘다.
- **스킨 시스템**: `assets/skins/<skinId>/skin.json` + PNG 프레임. 신규 스킨은 코드 수정 없이
  폴더/카탈로그 추가만으로 확장 가능하도록 설계.
- **가용성 판단 단일 지점**: `SkinAvailabilityChecker` 한 곳에서만 가용성(무료/개별구매/평생권/기프트코드)을 판단한다.
  Play 구매 동기화 결과와 기프트 해금은 출처를 분리해 환불 회수 범위를 명확히 유지한다.
...
- **결제 검증 Worker (Cloudflare Worker)**:
  - 엔드포인트: `https://daintyz-billing.xornexon.workers.dev`
  - 역할: 앱이 보낸 Play `purchaseToken`/평생이용권 토큰/기프트 토큰을 검증하고, 통과 시 비공개 R2의 유료 zip을 스트리밍.
  - 앱 연동: `billing/BillingConfig.kt`, `skin/SkinDownloader.downloadFromWorker`.
- **유료 zip 저장소 (Cloudflare R2)**:
  - 버킷: `daintyz-skins`
  - 키 형식: `{skinId}.zip`
  - 유료 테마 zip의 원본은 공개 `character/zip/`에 두지 않고 R2에 둔다.
...
설계 문서 7장 기준 1차 범위에서 출발했지만, 현재 코드는 결제/보호 다운로드까지 확장된 상태입니다.
아직 제외된 큰 축은 컨디션 분기, 다중 위젯 운영, 리사이즈 대응입니다.
찜/한정판매/기프트코드/인기 정렬용 스키마는 앱과 카탈로그에 미리 들어가 있습니다.
